...
public class AddCustomer extends JFrame implements ActionListener{
    ...
    AddCustomer(){
        ...
        ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icons/customer-removebg-preview.png"));
        JLabel image = new JLabel(i1);
        image.setBounds(250, 30, 400, 400);
        add(image);

        setBounds(300, 50, 600, 550);
        setVisible(true);
    }
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource() == add){
            ...
            try{
                Conn conn = new Conn();
                ResultSet rs = conn.s.executeQuery("DESCRIBE customer;");
                while(rs.next()){
                    System.out.println(rs.getString(1) + " " + rs.getString(2));
                }
                String query = "insert into customer values('" + id + "', '" + number + "', '" + name + "', '" + gender + "', '" + country + "', '" + room + "','" + formattedDate + "','" + deposit + "')";
